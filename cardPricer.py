import re, urllib2, math, datetime, time, os, sqlite3
...
def updateData(specific=""):
	...
	while i < pages:
		...
		try:
			...
			if "There was an error performing your search." in html:
				failed = True
		...
		if (i == 0 and specific == ""):
			pages = re.findall('"total_count":(\d+)', html)[0]
			pages = int(math.ceil(float(pages) / 95))
			print('there are %d pages' % pages)

		# regex data out
		names = re.findall('market_listing_item_name".*?>(.*?)<', html)
		games = re.findall('market_listing_game_name">(.*?)<', html)
		urls = re.findall('/listings/(\d+/.*?)">', html)
		prices = re.findall('&#36;(\d+.\d+)', html)
		counts = re.findall('market_listing_num_listings_qty">(.*?)<', html)
...
def updateSite():
	...
	cur.execute(q)
	a = cur.fetchone()
	o = o.replace('[EXPENSIVE-NAME]', a[1].encode('ascii', 'xmlcharrefreplace'))
...
################################################################################
# calls an external script to upload the generated page
def upload():
	print('uploading')
